...
function minimax(tBoard, roqueStats, depth, alpha, beta, isMaximizing) {
  let alp = alpha;
  let bet = beta;
  let gameState = checkmateAI(tBoard, roqueStats);
  if (depth == 0) {
    return calculateScore(tBoard);
  } else if (locateKing(player, tBoard) == 0) {
    return Infinity;
      ...
      for (let j = 0; j < 8; j++) {
        if (tBoard[i][j] != 0 && tBoard[i][j].substring(0, 5) == ai) {
          ...
          for (let k = 0; k < evMoves.length; k++) {
            ...
            bestValue = Math.max(
              bestValue,
              minimax(newTestBoard, newRoqueStats, depth - 1, alp, bet, false)
            );
      ...
      for (let j = 0; j < 8; j++) {
        if (tBoard[i][j] != 0 && tBoard[i][j].substring(0, 5) == player) {
          ...
          for (let k = 0; k < evMoves.length; k++) {
            ...
            bestValue = Math.min(
              bestValue,
              minimax(newTestBoard, newRoqueStats, depth - 1, alp, bet, true)
            );
